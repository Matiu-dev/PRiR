...
public class Filozof3 extends Thread{
    ...
    int mojNum; Random losuj ;

    public Filozof3 (int nr ) {
        mojNum=nr ; losuj = new Random(mojNum) ;
    }
    ...
    public void run ( ) {
        ...
        do {
            ...
            if( ! ( widelec [ (mojNum+1)%MAX].tryAcquire ( ) ) ) {
                widelec[mojNum].release ( ) ; } else { podnioslDwaWidelce = true ;
            }
        } else {
        ...
        widelec [mojNum].release ( ) ; widelec [ (mojNum+1)%MAX].release ( ) ;
    }
    }
}
